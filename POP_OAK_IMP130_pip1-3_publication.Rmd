...
```{r}
## for pipeline 1, Density cut-off


cm <- as.matrix(confusionMatrix(combine_OAK$cutoff, combine_OAK$Final.call))
n = sum(cm) # number of instances
nc = nrow(cm) # number of classes
rowsums = apply(cm, 1, sum) # number of instances per class
colsums = apply(cm, 2, sum) # number of predictions per class
...
```{r}
## for pipeline 1, Density cut-off
cm <- as.matrix(confusionMatrix(dat_IMP130$cutoff, dat_IMP130$Final.call))
n = sum(cm) # number of instances
nc = nrow(cm) # number of classes
rowsums = apply(cm, 1, sum) # number of instances per class
colsums = apply(cm, 2, sum) # number of predictions per class
